...
class Building(object):
    ...
    def __init__(self, floor_height, int_heat_night, int_heat_day, int_heat_frad, int_heat_flat,
                 infil, vent, glazing_ratio, u_value, shgc, condtype, cop, cool_setpoint_day,
                 cool_setpoint_night, heat_setpoint_day, heat_setpoint_night, coolcap, heateff,
                 initial_temp):

        self.floor_height = floor_height        # floor height
        self.int_heat = int_heat_night                 # timetep internal gains (W m-2 bld) (sensible only)
        self.int_heat_night = int_heat_night            # nighttime internal heat gains  (W m-2 floor)
        self.int_heat_day = int_heat_day                # daytime internal heat gains  (W m-2 floor)
        self.int_heat_frad = int_heat_frad              # internal gain radiant fraction
        self.int_heat_flat = int_heat_flat              # internal gain latent fraction
        self.infil = infil                          # Infiltration (ACH)
        self.vent = vent                            # Ventilation (m^3/s/m^2)
        self.glazing_ratio = glazing_ratio            # glazing ratio
        self.u_value = u_value                        # window U-value ( w m-2 K-1) including film coeff
        ...
        self.cool_setpoint_day = cool_setpoint_day      # daytime indoor cooling setpoint [K]
        self.cool_setpoint_night = cool_setpoint_night  # nighttime indoor heating setpoint [K]
        self.heat_setpoint_day = heat_setpoint_day      # daytimge indoor heating setpoint [K]
        self.heat_setpoint_night = heat_setpoint_night  # nighttime indoor heating setpoint [K]
        self.coolcap = coolcap                      # rated cooling system capacity (W m-2)
        self.heateff = heateff                      # heating system capacity (-)
        self.msys = coolcap / 1004. / (min(cool_setpoint_day, cool_setpoint_night) - 14 - 273.15) # HVAC supply mass flowrate (kg s-1 m-2)
        self.initial_temp = initial_temp
        self.indoor_temp = initial_temp               # Indoor Air Temperature [K]
        self.indoor_hum = 0.012                      # Indoor specific humidity [kgv/kga]
        self.heat_cap = 999                          # Default heat capacity value
        self.cop_adj = cop                           # adjusted COP per temperature
    ...
    def BEMCalc(self, UCM, BEM, forc, parameter, simTime):
        ...
        self.nFloor = max(UCM.bldHeight / float(self.floor_height), 1)   # At least one floor
        ...
        Qdehum = 0.0
        dens =  moist_air_density(forc.pres, self.indoor_temp, self.indoor_hum)# [kgv/ m-3] Moist air density given dry bulb temperature, humidity ratio, and pressure
        ...
        QLintload = self.int_heat * self.int_heat_flat # Qlatent Internal load = timestep internal gain * internal gain latent fraction
        ...
        self.sensHeatDemand = max(
            -(wallArea * zac_in_wall * (T_wall-T_heat) +  # wall load
              ...
              winArea * self.u_value * (T_can - T_heat) +  # window load due to temp delta
        ...
        if self.sensCoolDemand > 0. and UCM.canTemp > 288.:
            ...
            self.dehumDemand = max(VolCool * dens * (self.indoor_hum - 0.9 * 0.0078) * parameter.lv, 0.)
            ...
            self.coolConsump = (max(self.sensCoolDemand+self.dehumDemand,0.0))/self.cop_adj
        ...
        self.indoor_hum = self.indoor_hum + (simTime.dt / (dens * parameter.lv * UCM.bldHeight)) * \
                          (QLintload + QLinfil + QLvent - Qdehum)

        # Calculate relative humidity (Pw/Pws*100) using pressurce, indoor temperature, humidity
        _Tdb, _w, _phi, _h, _Tdp, _v = psychrometrics(self.indoor_temp, self.indoor_hum, forc.pres)
        ...
        self.fluxMass = zac_in_mass * (T_indoor - T_mass) + self.int_heat * self.int_heat_f_rad / massArea
        ...
        self.fluxInterior = self.int_heat * self.int_heat_f_rad * (1. - self.int_heat_flat) / self.nFloor
        ...
        # N.B No L to kg conversion for water in this equation because 1 L of water = 1 kg of water.
    ...
    def __repr__(self):
        return 'Building,\n floor_height: {}\n glazing_ratio: {}\n ' \
            'u_value: {}\n shgc: {}\n infil: {}\n vent: {}\n ' \
            'condtype: {}\n'.format(
                self.floor_height, self.glazing_ratio, self.u_value, self.shgc,
